...
| ۵ | `prisma migrate deploy` | **استقرار انجام نمی‌شود** |
| ۶ | `prisma generate` | **استقرار انجام نمی‌شود** |
| ۷ | `npm run lint` (type-check) | **استقرار انجام نمی‌شود** |
| ۸ | `npm run test:rules` (چک قواعد، حدود ۲ ثانیه) | **استقرار انجام نمی‌شود** |
| ۹ | `npm run build` | **استقرار انجام نمی‌شود** |
| ۱۰ | ری‌استارت سرویس | توقف با پیام |
| ۱۱ | بررسی سلامت روی پورت ۳۰۰۰ | راهنمای عیب‌یابی |

نکته‌ی کلیدی: تا وقتی type-check و چک قواعد و build سبز نشوند، سرویس **ری‌استارت نمی‌شود** — پس یک کامیت خراب نمی‌تواند برنامه را از کار بیندازد.

**کد خروج همان شماره‌ی مرحله‌ای است که متوقف شده** — مرحله‌ی ۹ با `9` بیرون می‌آید و `0` یعنی استقرار سالم تمام شد. پس `%ERRORLEVEL%` می‌گوید کجا ایستاد و پیام کنارش می‌گوید چرا. پیش از این دو تا از آن‌ها یکی بودند: هم شکست build و هم «پورت دست چیزی است که مال ما نیست» با `7` بیرون می‌آمدند، در حالی که پاسخ این دو کاملاً فرق دارد — یکی یک کامیت می‌خواهد و دیگری پیدا کردن یک پروسه روی سروری که با IIS و SQL Server و Report Server مشترک است.

### مرحله‌ی ۸ — چک قواعد

`npm run lint` فقط ایمپورت گم‌شده و متغیر بی‌استفاده را می‌بیند. یک hook زیر یک `return` زودهنگام، یک route که بعد از route شناسه ثبت شده و آن را می‌بلعد، یک migration با `GO` خالی، دو نسخه از یک قاعده که از هم جدا افتاده‌اند — همه‌ی این‌ها بی‌عیب type-check و build می‌شوند و هر کدام از آن‌ها قبلاً به همین سرور رسیده است. این suite نه دیتابیس می‌خواهد نه مرورگر، پس تنها چیزی است که می‌تواند بین یک کامیت و این سرور بایستد.

یک بار از اسکریپت برداشته شد و دلیلش هیچ‌وقت خود ایده نبود: این مخزن `.gitattributes` ندارد و Git for Windows فایل‌ها را با CRLF چک‌اوت می‌کند، پس پنج چک — آن‌هایی که الگویشان از یک خط رد می‌شود — روی درختی شکست می‌خوردند که کاراکتر‌به‌کاراکتر همان کامیتی بود که روی کامپیوتر توسعه سبز شده بود. حالا suite شکستن خط را همان‌جا که فایل را می‌خواند یکسان می‌کند، پس این سرور و آن کامپیوتر یک جواب می‌دهند. اگر این مرحله دوباره شکست خورد، دارد یک ایراد واقعی را نام می‌برد — همان چیزی را که چاپ کرده بخوانید.
