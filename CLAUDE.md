...
## SQL Server migration (in progress)

The app is being moved off `database.json` onto SQL Server. The relational schema is deployed and the first module is live; the legacy JSON store still backs everything else, so **both paths exist at once** until every module is converted.
...
- **Converted so far**: customers, projects, proformas, products/inventory. The remaining 14 modules still run on `database.json`.
