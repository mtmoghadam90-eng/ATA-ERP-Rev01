...
export function buildWorkflowDraftPrompt(
  ...
  return [
    ...
    // The reported fault, written as a rule because it is the one the model is
    // most likely to reproduce: the sentence a person says («یک هفته بعد از
    // ارسال، نتیجه را بپرس») names the event and never the state.
    "۴-الف) پیامی که از مشتری چیزی می‌پرسد، باید شرطِ «هنوز همان وضعیت برقرار",
    "   است» داشته باشد — نه فقط شرطِ رویدادی که قبلاً افتاده. شرط‌های یک قانون",
    "   زمان‌بندی‌شده در لحظهٔ اجرا روی رکورد سنجیده می‌شوند، پس این کار شدنی است",
    "   و نکردنش یعنی پیام برای کسی می‌رود که وضعیتش عوض شده.",
    "   مثال: «یک هفته پس از ارسال پیش‌فاکتور، نتیجهٔ بررسی را بپرس» بدون شرط،",
    "   برای مشتری‌ای هم می‌رود که همان هفته سفارشش را قطعی کرده. درستش:",
    "   time_elapsed + proforma_sent + ۷ روز + شرط‌های settled=false،",
    "   followUpState=OPEN، superseded=false و chaseCount کمتر از ۱.",
    "۴-ب) برای «چند روز بعد» وقتی وضعیت ممکن است در این فاصله عوض شود،",
    "   time_elapsed را ترجیح بده نه delayDays: delayDays پیام را همان لحظه در",
    "   صف می‌گذارد و دیگر هیچ شرطی دوباره سنجیده نمی‌شود.",
