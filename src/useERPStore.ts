...
export function useERPStore() {
  ...
  const addProduct = (
    ...
    // A code the user typed always wins. Testing for an "EQ-" prefix used to
    // discard user codes that happened to start with the default prefix.
  ...
  const addPurchaseOrder = (po: Omit<PurchaseOrder, "id">) => {
    // A number the user typed always wins. Testing for a "PO-" prefix used to
    // discard user numbers that happened to start with the default prefix.
    let finalCode = cleanCode(po.poNumber);
    if (!finalCode) {
      finalCode = formatERPNumber(
  ...
  const newProject: Project = {
    ...project,
    id: `proj-${Date.now()}`,
    // A code typed by the user wins; the generated one is only the default.
    code: cleanCode((project as any).code) || computedCode,
  ...
  const addProforma = (proforma: Omit<Proforma, 'id' | 'proformaNumber'> & { proformaNumber?: string }) => {
  ...
  const newProforma: Proforma = {
    ...proforma,
    id: `pf-${Date.now()}`,
    proformaNumber: cleanCode((proforma as any).proformaNumber) || computedNumber,
  ...
  const addPackagingDelivery = (pd: any) => {
    ...
    const newPd = {
      ...pd,
      id: `pd-${Date.now()}`,
      packingListNumber: cleanCode(pd.packingListNumber) || packingListNumber,
      createdAt: new Date().toISOString(),
    };
